...
uint8_t i2c_24c02_test(void)
{
    ...
    for(i = 0; i < BUFFER_SIZE; i++) {
        ...
        u0_printf("0x%02X ", i2c_buffer_write[i]);
        if(15 == i % 16) {
            u0_printf("\r\n");
        }
    }
    ...
    u0_printf("AT24C02 reading...\r\n");
    /* EEPROM data read */
    eeprom_buffer_read_timeout(i2c_buffer_read, EEP_FIRST_PAGE, BUFFER_SIZE);
    /* compare the read buffer and write buffer */
    for(i = 0; i < BUFFER_SIZE; i++) {
        if(i2c_buffer_read[i] != i2c_buffer_write[i]) {
            u0_printf("0x%02X ", i2c_buffer_read[i]);
            u0_printf("Err:data read and write aren't matching.\n\r");
            return I2C_FAIL;
        }
        u0_printf("0x%02X ", i2c_buffer_read[i]);
        if(15 == i % 16) {
    ...
    u0_printf("I2C-AT24C02 test passed!\n\r");
    return I2C_OK;
}
    ...
    while(!(i2c_timeout_flag)) {
        ...
        case I2C_START:
            ...
            if(timeout < I2C_TIME_OUT) {
                ...
                timeout = 0;
                state   = I2C_START;
                u0_printf("i2c bus is busy in WRITE BYTE!\n");
            }
            break;
        case I2C_SEND_ADDRESS:
            ...
            if(timeout < I2C_TIME_OUT) {
                ...
                timeout = 0;
                state = I2C_START;
                u0_printf("i2c master sends start signal timeout in WRITE BYTE!\n");
            }
            break;
        case I2C_CLEAR_ADDRESS_FLAG:
            ...
            if(timeout < I2C_TIME_OUT) {
                ...
                timeout = 0;
                state = I2C_START;
                u0_printf("i2c master clears address flag timeout in WRITE BYTE!\n");
            }
            break;
        case I2C_TRANSMIT_DATA:
            ...
            if(timeout < I2C_TIME_OUT) {
                ...
                timeout = 0;
                state = I2C_START;
                u0_printf("i2c master sends data timeout in WRITE BYTE!\n");
            }
        ...
        case I2C_STOP:
            ...
            if(timeout < I2C_TIME_OUT) {
                ...
                timeout = 0;
                state = I2C_START;
                u0_printf("i2c master sends stop signal timeout in WRITE BYTE!\n");
            }
            break;
        default:
            state = I2C_START;
            i2c_timeout_flag = I2C_OK;
            timeout = 0;
            u0_printf("i2c master sends start signal in WRITE BYTE.\n");
            break;
        }
    }
    return I2C_END;
    ...
    while(!(i2c_timeout_flag)) {
        ...
        case I2C_START:
            ...
            if(timeout < I2C_TIME_OUT) {
                ...
                timeout = 0;
                state = I2C_START;
                u0_printf("i2c bus is busy in WRITE!\n");
            }
            break;
        case I2C_SEND_ADDRESS:
            /* i2c master sends START signal successfully */
            ...
            if(timeout < I2C_TIME_OUT) {
                ...
                timeout = 0;
                state = I2C_START;
                u0_printf("i2c master sends start signal timeout in WRITE!\n");
            }
            break;
        case I2C_CLEAR_ADDRESS_FLAG:
            /* address flag set means i2c slave sends ACK */
            ...
            if(timeout < I2C_TIME_OUT) {
                ...
                timeout = 0;
                state = I2C_START;
                u0_printf("i2c master clears address flag timeout in WRITE!\n");
            }
            break;
        case I2C_TRANSMIT_DATA:
            /* wait until the transmit data buffer is empty */
            ...
            if(timeout < I2C_TIME_OUT) {
                ...
                timeout = 0;
                state = I2C_START;
                u0_printf("i2c master sends EEPROM's internal address timeout in WRITE!\n");
            }
            /* wait until BTC bit is set */
            while((!i2c_flag_get(I2CX, I2C_FLAG_BTC)) && (timeout < I2C_TIME_OUT)) {
                timeout++;
            ...
            if(timeout < I2C_TIME_OUT) {
                ...
                timeout = 0;
                state = I2C_START;
                u0_printf("i2c master sends data timeout in WRITE!\n");
            }
        ...
        case I2C_STOP:
            ...
            if(timeout < I2C_TIME_OUT) {
                ...
                timeout = 0;
                state = I2C_START;
                u0_printf("i2c master sends stop signal timeout in WRITE!\n");
            }
            break;
        default:
            state = I2C_START;
            i2c_timeout_flag = I2C_OK;
            timeout = 0;
            u0_printf("i2c master sends start signal in WRITE.\n");
            break;
        }
    }
    return I2C_END;
    ...
    while(!(i2c_timeout_flag)) {
        ...
        case I2C_START:
            if(RESET == read_cycle) {
                ...
                if(timeout < I2C_TIME_OUT) {
                    ...
                    timeout = 0;
                    state = I2C_START;
                    u0_printf("i2c bus is busy in READ!\n");
                }
            }
        ...
        case I2C_SEND_ADDRESS:
            ...
            if(timeout < I2C_TIME_OUT) {
                ...
                state = I2C_START;
                read_cycle = 0;
                u0_printf("i2c master sends start signal timeout in READ!\n");
            }
            break;
        case I2C_CLEAR_ADDRESS_FLAG:
            /* address flag set means i2c slave sends ACK */
            ...
            if(timeout < I2C_TIME_OUT) {
                ...
                state   = I2C_START;
                read_cycle = 0;
                u0_printf("i2c master clears address flag timeout in READ!\n");
            }
            break;
        case I2C_TRANSMIT_DATA:
            if(RESET == read_cycle) {
                ...
                if(timeout < I2C_TIME_OUT) {
                    ...
                    state = I2C_START;
                    read_cycle = 0;
                    u0_printf("i2c master wait data buffer is empty timeout in READ!\n");
                }
                /* wait until BTC bit is set */
                while((!i2c_flag_get(I2CX, I2C_FLAG_BTC)) && (timeout < I2C_TIME_OUT)) {
                    timeout++;
                ...
                if(timeout < I2C_TIME_OUT) {
                    ...
                    state = I2C_START;
                    read_cycle = 0;
                    u0_printf("i2c master sends EEPROM's internal address timeout in READ!\n");
                }
            } else {
                while(number_of_byte) {
                    timeout++;
                    ...
                    if(timeout > I2C_TIME_OUT) {
                        ...
                        state = I2C_START;
                        read_cycle = 0;
                        u0_printf("i2c master sends data timeout in READ!\n");
                    }
                }
                timeout = 0;
                state = I2C_STOP;
        ...
        case I2C_STOP:
            ...
            if(timeout < I2C_TIME_OUT) {
                ...
                state = I2C_START;
                read_cycle = 0;
                u0_printf("i2c master sends stop signal timeout in READ!\n");
            }
            break;
        default:
            state = I2C_START;
            ...
            i2c_timeout_flag = I2C_OK;
            timeout = 0;
            u0_printf("i2c master sends start signal in READ.\n");
            break;
        }
    }
    return I2C_END;
    ...
    while(!(i2c_timeout_flag)) {
        ...
        case I2C_START:
            ...
            if(timeout < I2C_TIME_OUT) {
                ...
                timeout = 0;
                state = I2C_START;
                u0_printf("i2c bus is busy in EEPROM standby!\n");
            }
            break;
        case I2C_SEND_ADDRESS:
            /* i2c master sends START signal successfully */
            ...
            if(timeout < I2C_TIME_OUT) {
                ...
                timeout = 0;
                state = I2C_START;
                u0_printf("i2c master sends start signal timeout in EEPROM standby!\n");
            }
            break;
        case I2C_CLEAR_ADDRESS_FLAG:
            ...
            if(timeout < I2C_TIME_OUT) {
                ...
                timeout = 0;
                state = I2C_START;
                u0_printf("i2c master clears address flag timeout in EEPROM standby!\n");
            }
            break;
        case I2C_STOP:
            /* send a stop condition to I2C bus */
            ...
            if(timeout < I2C_TIME_OUT) {
                ...
                timeout = 0;
                state = I2C_START;
                u0_printf("i2c master sends stop signal timeout in EEPROM standby!\n");
            }
            break;
        default:
            state = I2C_START;
            timeout = 0;
            u0_printf("i2c master sends start signal end in EEPROM standby!.\n");
            break;
        }
    }
    return I2C_END;
